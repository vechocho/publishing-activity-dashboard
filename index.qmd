...
# 2. Get metadata for each key and process it into the final 'datasets' table
datasets <- ecuador_dataset_keys %>%
  map_dfr(~{
    ...
    tibble(
      ...
      created = meta$data$created,  # We get 'created' date here
      modified = meta$data$modified # We get 'modified' date here
    )
  }) %>%
  # 3. Process the dates
  mutate(
